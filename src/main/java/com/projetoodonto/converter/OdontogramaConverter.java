package com.projetoodonto.converter;

import javax.faces.component.UIComponent;
import javax.faces.context.FacesContext;
import javax.faces.convert.Converter;
import javax.faces.convert.FacesConverter;
import javax.inject.Inject;

import com.projetoodonto.dao.OdontogramaDAO;
import com.projetoodonto.model.Cliente;
import com.projetoodonto.model.Odontograma;

@FacesConverter(forClass=Odontograma.class)
public class OdontogramaConverter implements Converter {
	@Inject
	private OdontogramaDAO odontogramaDAO;
	
	
	@Override
	public Object getAsObject(FacesContext context, UIComponent component, String value) {
		Odontograma odontograma = null;
		
		if(value != null) {
			return odontograma = this.odontogramaDAO.buscarPorCodigo(Long.parseLong(value));
		}
		
		return odontograma;
	}

	@Override
	public String getAsString(FacesContext context, UIComponent component, Object value) {
		if(value != null) {
			Long codigo = ((Odontograma) value).getCodigo();//Faz o cast de objeto para long.
			return codigo == null ? null : codigo.toString();//Retorna o codigo
		}
		return null;
	}
}
